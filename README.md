...
### Ari_msg.py
 ...
 Ari_slena에서 쓸 첨부물/일반 메세지를 모아둔 파일이다. (Ari_slena의 명령어).(용도) 형식으로 이름을 정했으며, Foo.bar 형식이면 해당 str을 바로 가져올 수 있다.
...
NationInfo : 나라명, 오너명, 현재 자금, 현재 빚, 남은 상환 기간, 가용 정책 점수, 금일 행동 횟수, 유저 여부, 호전성 수치
...
`area_standard_relation ` : 

`nation_standard_relation ` : 
